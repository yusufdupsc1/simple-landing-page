...
export function AnalyticsClient({
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: formatCurrency(financeSummary.paidFees.amount),
      color: "text-foreground",
    },
    {
      label: "Outstanding Fees",
      value: formatCurrency(financeSummary.pendingFees.amount),
      color: "text-yellow-600",
    },
    {
      label: "Overdue Fees",
      value: financeSummary.overdueCount,
  ...
  return (
    <>
      <PageHeader
        title="Analytics"
        description="Institution-wide insights across all modules"
      />
        ...
        {attendanceAnomalies.length > 0 ? (
          <ul className="mt-3 space-y-2">
            {attendanceAnomalies.map((anomaly) => (
              ...
              >
                Attendance dip on {format(new Date(anomaly.date), "MMM d")}:{" "}
                {anomaly.count} present vs {anomaly.average} average.
              </li>
            ))}
          </ul>
        ) : null}
      ...
      <div className="rounded-xl border border-border bg-card p-4 sm:p-5">
        ...
        {trendData.length > 0 ? (
          ...
          <div className="h-60 flex items-center justify-center text-muted-foreground text-sm">
            No attendance data in the last 14 days.
          </div>
        )}
      </div>
            ...
            >
              ...
              <Bar dataKey="value" radius={[6, 6, 0, 0]}>
                {financeData.map((_, i) => (
                  <Cell
                    key={i}
                    fill={i === 0 ? "#6366f1" : i === 1 ? "#22c55e" : "#f59e0b"}
                  />
                ))}
              </Bar>
            </BarChart>
