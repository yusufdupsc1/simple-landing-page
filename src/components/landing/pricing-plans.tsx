...
export function PricingPlans() {
  return (
    <section id="pricing" className="mx-auto w-full max-w-6xl px-4 py-16 sm:px-8">
      <div className="flex flex-col items-center text-center mb-12">
        <h2 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">Simple Govt Primary Pricing</h2>
        ...
        {pricingTiers.map((tier) => (
          ...
          >
            ...
            <div className="mt-4 flex items-baseline gap-1">
              <p className="text-4xl font-extrabold tabular-nums text-foreground">{tier.price}</p>
              <p className="text-sm font-medium text-muted-foreground">{tier.cadence}</p>
            </div>

            <div className="mt-8 space-y-4 flex-1">
              {tier.features.map((feature) => (
                <div key={feature} className="flex items-start gap-3 text-sm text-foreground/80 leading-snug">
            ...
            <Button
              ...
              className={[
                ...
                tier.highlighted ? "primary-cta" : "bg-card/80 border-border/60 hover:bg-muted",
              ].join(" ")}
