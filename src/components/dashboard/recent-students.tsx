...
import { UserPlus, ArrowRight, ArrowUpRight } from "lucide-react";
    ...
    <section className="group relative flex h-full flex-col overflow-hidden rounded-[2rem] border border-border/40 bg-card/60 backdrop-blur-xl p-6 shadow-sm transition-premium hover:border-primary/20 premium-shadow">
      <div className="flex items-center justify-between mb-6">
        <div className="flex flex-col">
          <h2 className="text-lg font-black tracking-tight text-foreground/90">New Admissions</h2>
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Recent Enrollments</p>
        </div>
        <Link href="/dashboard/students" className="text-[11px] font-bold text-muted-foreground hover:text-primary flex items-center gap-1 transition-colors uppercase tracking-widest leading-none">
      ...
      <div className="space-y-3 flex-1">
        {students.length ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-tight">
                    ID: {student.studentId} {student.class?.name ? `• ${student.class.name}` : ""}
                  </p>
                </div>
              </div>
          ...
          <div className="h-48 flex flex-col items-center justify-center p-6 border-2 border-dashed border-border/60 rounded-[1.5rem] bg-muted/10">
            ...
            <p className="text-sm font-bold text-muted-foreground/60">No recent enrollments</p>
            <p className="text-[10px] text-muted-foreground/40 mt-1 uppercase tracking-widest text-center">New student data will appear here</p>
