...
function AnnouncementForm({
  ...
  const [form, setForm] = useState<AnnouncementFormData>({
    ...
    priority:
      (initial?.priority as AnnouncementFormData["priority"]) ?? "NORMAL",
    targetAudience: initial?.targetAudience ?? ["ALL"],
    expiresAt: initial?.expiresAt
      ? new Date(initial.expiresAt).toISOString().slice(0, 10)
      : "",
  });
  const set = (k: keyof AnnouncementFormData, v: unknown) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        toast.success(
          initial ? "Announcement updated" : "Announcement published",
        );
        onSuccess();
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Select
            value={form.priority}
            onValueChange={(v) =>
              set("priority", v as AnnouncementFormData["priority"])
            }
          >
            <SelectTrigger>
              <SelectValue />
      ...
      <div className="space-y-3">
        {announcements.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border p-12 text-center text-muted-foreground">
            No announcements yet. Click &ldquo;New Announcement&rdquo; to
            publish one.
          </div>
        ) : (
          announcements.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className={`rounded-xl border bg-card p-4 transition-shadow hover:shadow-sm ${isExpired ? "opacity-60" : ""}`}
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${cfg.bg} ${cfg.color}`}
                      >
                        {cfg.label}
                      </span>
                      {isExpired && (
