...
function GradeForm({
  ...
  const set = (k: keyof GradeFormData, v: unknown) =>
    setForm((f) => ({ ...f, [k]: v }));

  const percentage =
    form.maxScore > 0
      ? Math.round((Number(form.score) / Number(form.maxScore)) * 100)
      : 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = initial
        ? await updateGrade(initial.id, form)
        : await createGrade(form);
      if (res.success) {
        toast.success(initial ? "Grade updated" : "Grade recorded");
        onSuccess();
      ...
      <PageHeader title="Grades" total={total} totalLabel="records">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editGrade ? "Edit Grade" : "Record Grade"}
              </DialogTitle>
            </DialogHeader>
            <GradeForm
            ...
            <div className="-mx-4 overflow-x-auto px-4 sm:mx-0 sm:px-0">
              <table className="w-full min-w-[640px] text-sm">
                <thead className="border-b border-border bg-muted/30">
                  <tr>
                    {["Student", "Subject", "Score", "Grade", "Term", ""].map(
                      (h) => (
                        <th
                          key={h}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            ...
                            style={{
                              background: `${GRADE_COLORS[g.letterGrade ?? ""] ?? "#94a3b8"}20`,
                              color:
                                GRADE_COLORS[g.letterGrade ?? ""] ?? "#94a3b8",
                            }}
                          >
                            {g.letterGrade ?? "—"}
                          </span>
