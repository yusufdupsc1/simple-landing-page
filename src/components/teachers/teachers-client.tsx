...
function TeacherForm({
  ...
  const set = (k: keyof TeacherFormData, v: unknown) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        toast.success(
          initial ? `${singularLabel} updated` : `${singularLabel} created`,
        );
        const credential =
          !initial &&
          res.data &&
          typeof res.data === "object" &&
          "credential" in res.data
            ? (res.data as { credential?: { email: string; password: string } })
                .credential
            : null;
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v) => set("status", v as TeacherFormData["status"])}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            ...
            >
              ...
              {govtPrimaryMode ? "Add Assistant Teacher" : "Add Teacher"}
            </Button>
          </DialogTrigger>
        ...
        <div className="-mx-4 overflow-x-auto px-4 sm:mx-0 sm:px-0">
          <table className="w-full min-w-[700px] text-sm">
            <thead className="border-b border-border bg-muted/30">
              <tr>
                {[
                  govtPrimaryMode ? "Assistant Teacher" : "Teacher",
                  "ID",
                  "Contact",
                  "Specialization",
                  "Classes",
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        {t.classTeacher.length > 0
                          ? t.classTeacher.map((c) => c.name).join(", ")
                          : "—"}
                      </div>
                    </td>
