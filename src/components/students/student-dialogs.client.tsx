...
type CaseColor = { id: string; name: string; grade: string; section: string };
...
export function StudentDialogs({
    ...
    return (
        <>
            <Dialog open={dialogParam === "create" || !!editParam} onOpenChange={(open) => !open && closeDialog()}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editParam ? "Edit Student" : "Add New Student"}</DialogTitle>
                        <DialogDescription>
                            {editParam ? "Update profile details." : "Create a new student record."}
                        </DialogDescription>
                    </DialogHeader>
